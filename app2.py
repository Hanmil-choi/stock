    ...
    if not selected_codes or not conditions:
        ...
        for i, (d_start, d_end) in enumerate(date_ranges):
            ...
            if market_hold_condition.strip():
                try:
                    # 사이클 기간의 첫 번째 종목 데이터프레임을 사용하여 조건 평가
            ...
            for code in selected_codes:
                try:
                    ...
                    # 가격 데이터 추출 - 마지막날 데이터가 없으면 이전 날 데이터 사용
        ...
        if equity_curve:
            ...
            if "069500" in stock_codes:
                ...
                if len(df_kodex) >= 2:
                    start_price = df_kodex.iloc[0]["close"]
                    end_price = df_kodex.iloc[-1]["close"]
                    buyhold_return = round((end_price - start_price) / start_price * 100, 2)
                    # MDD 계산
                    values = df_kodex["close"].values
                    mdd = 0
                    ...
                    st.metric("Buy and Hold (KODEX 200) Return", f"{buyhold_return}%")
                    st.metric("Buy and Hold (KODEX 200) MDD", f"{buyhold_mdd}%")
